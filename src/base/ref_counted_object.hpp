...
public:
    ...
    template<typename U>
    RefCountedObject(U&& obj) : T(std::forward<U>(obj)){

    }

    template<typename P0,typename P1,typename ...Args>
    RefCountedObject(P0 &&p0,P1 &&p1,Args&& ...args) : T(std::forward<P0>(p0),
                                                         std::forward<P1>(p1),
    ...
    virtual bool HasOneRef() const { return ref_count_.load(std::memory_order_acquire) == 1; }
protected:
